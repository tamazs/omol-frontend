<template>
  ...
  <div class="form-container" id="sendLove">
    <form @submit.prevent="validateAndSend" id="form">
      ...
      <input type="email" v-model="email" name="from_email" :placeholder="$t('home.emailPh')"
        :class="{ 'red-border': !emailFilled && formSubmitted }" required>
      ...
      <textarea name="message" v-model="message" cols="1" rows="20" :placeholder="$t('home.msgPh')"
        :class="{ 'red-border': !messageFilled && formSubmitted }" required></textarea>
      ...
      <input type="submit" value="Send" @mouseenter="playSound" @mouseleave="stopSound" @mousedown="playMouseDownSound" @mouseup="stopMouseDownSound">
    </form>
  </div>
</template>  
